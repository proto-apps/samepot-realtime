/**
 * @fileoverview RedisStore for Socket.IO.
 * @author yo_waka
 */
...
var Store = require('socket.io/lib/store');
 ...
 *     - db (number) db number to be used by all redis clients, defaults to 0
...
function Redis (opts) {
  ...
  if (opts.db) {
      // In internal, the command is managed by queue.
      // So, you do not be aware of whether asynchronous command.
      this.pub.select(opts.db);
      this.sub.select(opts.db);
      this.cmd.select(opts.db);
  }
